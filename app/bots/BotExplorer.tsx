  ...
  return (
    <main>
      <header className="hero">
        ...
        {formattedDate && <p className="generated">Dernière analyse : {formattedDate}</p>}
        {data.social?.simulated && (
          <p className="sim-banner">
            🔬 Le « buzz » X est actuellement <strong>simulé</strong> (mock).
          </p>
        )}
        {data.backtest?.simulated && (
          <p className="sim-banner">
            📈 Les « profits » sont des <strong>backtests simulés</strong> out-of-sample — pas des gains réels.
          </p>
        )}
      </header>
...
function BotCard({ bot, rank }: { bot: Bot; rank: number }) {
  ...
  return (
    <a className="card" href={`https://github.com/${bot.fullName}`} target="_blank" rel="noreferrer">
      ...
      <h3 className="name">{bot.fullName}{profit && <span className="profit" title="stratégie orientée profit">💰</span>}</h3>
      <p className="desc">{bot.description || "—"}</p>
      <div className="score-row">
        <div className="score-bar"><div className="score-fill" style={{ width: `${bot.botScore}%` }} /></div>
      ...
      <div className="meta">
        ...
        {bot.backtestable && typeof bot.backtestReturn === "number" && (
          <span className={`bt ${bot.backtestReturn >= 0 ? "bt-pos" : "bt-neg"}`}
            title={`backtest ${bot.backtestStrategy}@${bot.backtestMarket} · sharpe ${bot.backtestSharpe} · drawdown ${Math.round((bot.backtestDrawdown ?? 0) * 100)}%`}>
            📈 {(bot.backtestReturn * 100).toFixed(1)}% sim
          </span>
        )}
        {typeof bot.socialScore === "number" && bot.mentions ? (
          <span title={`${bot.mentions} mentions X · ${Math.round((bot.profitRatio ?? 0) * 100)}% claims de profit`}>🔥 buzz {bot.socialScore}</span>
        ) : (
