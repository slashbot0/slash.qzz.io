  ...
  return (
    <main>
      <header className="hero">
        <Link className="backlink" href="/backtests">← retour au leaderboard</Link>
        <h1>{bot.fullName} <span className={`badge ${badge.cls}`}>{badge.label}</span></h1>
      ...
      <section className="panel">
        ...
        <div className="tags">
          {bot.strategies.map((s) => <span key={s} className="tag tag-strat">{s}</span>)}
          {bot.chains.map((c) => <span key={c} className="tag tag-chain">{c}</span>)}
          {bot.dexes.map((d) => <span key={d} className="tag tag-dex">{d}</span>)}
        </div>
        <p className="meta-line">⭐ {bot.stars.toLocaleString("fr-FR")} · {bot.language || "—"} · <a href={`https://github.com/${bot.fullName}`} target="_blank" rel="noreferrer">voir sur GitHub →</a></p>
      </section>

      <footer className="footer">⚠️ Backtests simulés — ne préjugent pas des résultats futurs. Ne jamais exécuter un bot tiers avec une vraie clé privée.</footer>
    </main>
  )
}
